...
<form action="/store" method="POST">
    ...
    <div class="max-w-md mx-auto shadow-lg p-5 my-5 flex flex-col bg-slate-100">
        ...
        <input name="name" class="rounded shadow-md outline-blue-500 p-2 border-slate-300" type="text">
        @error('name')
            <p class="text-red-500 text-sm font-bold">{{ $message }}</p>
        @enderror

        <label class="mt-3" for="slug">Slug</label>
        <input name="slug" class="rounded shadow-md outline-blue-500 p-2 border-slate-300" type="text">
        @error('slug')
            <p class="text-red-500 text-sm font-bold">{{ $message }}</p>
        @enderror

        <label class="mt-3" for="Start Date">Start Date</label>
        <input name="startAt" class="rounded shadow-md outline-blue-500 p-2 border-slate-300" type="datetime-local">
        @error('startAt')
            <p class="text-red-500 text-sm font-bold">{{ $message }}</p>
        @enderror

        <label class="mt-3" for="End Date">End Date</label>
        <input name="endAt" class="rounded shadow-md outline-blue-500 p-2 border-slate-300" type="datetime-local">
        @error('endAt')
            <p class="text-red-500 text-sm font-bold">{{ $message }}</p>
        @enderror
